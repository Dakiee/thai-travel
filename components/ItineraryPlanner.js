...
export default function ItineraryPlanner() {
  ...
  return (
    <>
      ...
      <p className="section-description">Enough structure to keep us organized, with plenty of room to change our minds.<br /><span>Amounts on each day are rough transport or activity costs per person.</span></p>
      <div className="filter-row" role="group" aria-label="Filter itinerary">{filters.map(([value, label]) => <button type="button" className={`filter ${filter === value ? 'active' : ''}`} aria-pressed={filter === value} onClick={() => setFilter(value)} key={value}>{label}</button>)}</div>
      <div>{current.plan.map((day, index) => {
